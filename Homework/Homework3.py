...
for i in range(1, len(lst)):
    if lst[i] == x:
        k += 1
print(f'Число х встречается {k} раз(-а)')
...
x = int(input('Введите число x = '))
for i in range(1, len(lst)):
    a=[abs(lst[i]-x) for i in range(len(lst))]
print(f'Самый близкий по величине элемент к числу {x}: {lst[a.index(min(a))]}')
...
print(f'Сумма очков: {sum(points[letter] for letter in text)}')